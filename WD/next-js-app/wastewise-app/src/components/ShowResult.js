...
const ShowResult = ({image, category}) => {

    const output = [
        {
            class: 'm-fail',
            h: 'Sorry, we couldn’t find a match. 😞',
            p: 'Please try again. Make sure you only scan a single item at a time. Also placing the object on a plain background like a tabletop or a wall can help us make a suggestion.'
        },
        {   
            class: 'm-cat1',
    ...
    return (
        <main className="showresult">
            <div className={`showresult__flexbox ${result.class}`}>
                <img src={image} alt="Your captured photo" className="showresult__photo" />
                <div className="showresult__text-box">
                    <h3>{result.h}</h3>
                    <p>{result.p}</p>
                </div>
            </div>
            <Link href="/scan" className="showresult__button">
